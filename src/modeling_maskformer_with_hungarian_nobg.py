...
def angular_loss_angles(inputs: Tensor, labels: Tensor, num_masks: int
) -> Tensor:
    num_images = int(int(num_masks.item()) / 3)
    inputs_rsh = torch.reshape(inputs, (-1, 3, inputs.shape[1], inputs.shape[2]))
    labels_rsh = torch.reshape(labels, (-1, 3, labels.shape[1], labels.shape[2]))
    cos_rsh = nn.CosineSimilarity(dim=1, eps=1e-6)
    loss = cos_rsh(inputs_rsh, labels_rsh)
    loss = torch.clamp(loss, min = -1.0 + 1e-6, max = 1.0 - 1e-6)
    loss = torch.acos(loss)
    loss = (loss * 180) / torch.pi
    loss = torch.reshape(loss, (num_images, -1)).mean(1).sum() / num_images
    return loss

def angular_loss_radians(inputs: Tensor, labels: Tensor, num_masks: int
) -> Tensor:
    num_images = int(int(num_masks.item()) / 3)
    inputs_rsh = torch.reshape(inputs, (-1, 3, inputs.shape[1], inputs.shape[2]))
    labels_rsh = torch.reshape(labels, (-1, 3, labels.shape[1], labels.shape[2]))
...
def mse_loss(inputs: Tensor, labels: Tensor, num_masks: int
) -> Tensor:
    criterion = nn.MSELoss(reduction="none")
    loss = criterion(inputs.flatten(1), labels.flatten(1))
...
class MaskFormerLoss(nn.Module):
    ...
    def loss_masks(
        ...
        #(tensor([0, 0, 0, 1, 1, 1]), tensor([1, 0, 2, 1, 0, 2]))
        #batch, and indices
        ...
        # check matching:
        # (target_masks[0] == mask_labels[0][1]).all()
        # (pred_masks[1] == masks_queries_logits[0][1]).all()
        # it looks like we change the order of ground truth labels
        # that should not really hurt the angle calculations
        ...
        losses = {
            #"loss_mask": angular_loss_radians(pred_masks, target_masks, num_masks)
            "loss_mask": mse_loss(pred_masks, target_masks, num_masks)
        }

        if torch.isnan(losses["loss_mask"]):
            print("Stop Here")
    ...
    def forward(
        ...
        WITH_FIXED_LABELS = True
        WITH_HUNGARIAN = True
        if WITH_FIXED_LABELS == True:
            ...
            losses: Dict[str, Tensor] = {
                ...
                **self.loss_labels_empty(class_queries_logits, class_labels, indices),
            }
        else:
            # retrieve the matching between the outputs of the last layer and the labels
            indices = self.matcher(masks_queries_logits, class_queries_logits, mask_labels, class_labels)
...
class MaskFormerForInstanceSegmentation(MaskFormerPreTrainedModel):
    def __init__(self, config: MaskFormerConfig):
        ...
        self.class_predictor = nn.Linear(hidden_size, config.num_labels)
